...
def fetch_ol_work(*, isbn: str | None, title: str, author: str) -> dict | None:
    ...
    data = cached_fetch("open_library_work", cache_key, lambda: _fetch(url), url=url)
    if not data:
        return None
...
def extract_useful_fields(work: dict, current_book: dict) -> dict:
    ...
    # ISBN — only fill if missing
    if not current_book.get("isbn"):
        isbns = work.get("isbn") or []
        # Prefer 13-digit ISBNs (newer standard); fall back to 10-digit
        isbn13 = [i for i in isbns if len(i) == 13]
        chosen = isbn13[0] if isbn13 else (isbns[0] if isbns else None)
        if chosen:
            out["isbn"] = chosen

    # DDC + LCC — array fields, store as-is (additive_merge handles arrays)
    if work.get("ddc"):
        out["ddc"] = work["ddc"]
    if work.get("lcc"):
        out["lcc"] = work["lcc"]

    # Subject_facet — cleaner than the raw `subject` array (less noise)
    if work.get("subject_facet"):
        out["subject_facet"] = work["subject_facet"]

    # OL work key for future reference — useful when we want to fetch more
